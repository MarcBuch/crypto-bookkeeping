...
program
  ...
  .action(async (tokenId?: string) => {
    ...
    const displayData: PnLDisplayData[] = pnlData.map((pos) => ({
      tokenId: pos.tokenId,
      pair: pos.pair,
      ...
      feesEarned: `${formatNumber(pos.feesCollected0, 4)} ${pos.token0Symbol} + ${formatNumber(pos.feesCollected1, 4)} ${pos.token1Symbol} (= ${formatNumber(pos.feesValueInToken1, 4)} ${pos.token1Symbol})`,
    ...
    output({ positions: pnlData }, () => displayPnL(displayData));
  });
