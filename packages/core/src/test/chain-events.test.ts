...
const COLLECT_TOPIC =
  "0x40d0efd1a53d60ecbf40971b9daf7dc90178c3aadc7aab1765632738fa8b8f01" as Hex;
...
function clientForLogs(logs: Array<ReturnType<typeof decreaseLog> | ReturnType<typeof collectLog>>) {
  const receipt = {
    blockNumber: 458834n,
...
describe("findCloseEventFromTx", () => {
  it("decodes the observed close shape with DecreaseLiquidity followed by Collect", async () => {
    const event = await findCloseEventFromTx(
      clientForLogs([
        decreaseLog(123n, 1000n, 2000n, 50n),
        collectLog(123n, 1010n, 2050n),
      ]),
      TX_HASH,
      123n
    );
  ...
  it("subtracts principal from Collect amounts to return raw collected fees", async () => {
    const event = await findCloseEventFromTx(
      clientForLogs([
        decreaseLog(123n, 10_000n, 20_000n),
        collectLog(123n, 10_123n, 20_456n),
      ]),
      TX_HASH,
      123n
    );
